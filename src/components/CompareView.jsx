...
// Stars to Forks Ratio static chart component
function StarsForksRatioChart({ selectedRepos, repoData }) {
  ...
  // Build comparison data with stars/forks ratio
    ...
    if (viewMode === 'indexed') {
      ...
      for (let i = 0; i < maxLength; i++) {
        ...
        selectedRepos.forEach(repoKey => {
          const data = filteredData[repoKey] || [];
          if (i < data.length) {
            const stars = data[i].totalStars || 0;
            const forks = data[i].totalForks || 0;
            point[repoKey] = forks > 0 ? stars / forks : null;
          }
        });
      ...
      return sortedDates.map(date => {
        ...
        selectedRepos.forEach(repoKey => {
          ...
          if (dayData) {
            const stars = dayData.totalStars || 0;
            const forks = dayData.totalForks || 0;
            point[repoKey] = forks > 0 ? stars / forks : null;
          }
        });
        return point;
...
// Stars to Contributors Ratio static chart component
function StarsContributorsRatioChart({ selectedRepos, repoData }) {
  const [datePreset, setDatePreset] = useState('all');
  const [customStartDate, setCustomStartDate] = useState('');
  const [customEndDate, setCustomEndDate] = useState('');
    ...
    if (viewMode === 'indexed') {
      ...
      for (let i = 0; i < maxLength; i++) {
        ...
        selectedRepos.forEach(repoKey => {
          ...
          if (i < data.length) {
            const stars = data[i].totalStars || 0;
            const contributors = data[i].totalContributors || 0;
            point[repoKey] = contributors > 0 ? stars / contributors : null;
          }
        });
        indexedData.push(point);
      }
      ...
      return sortedDates.map(date => {
        ...
        selectedRepos.forEach(repoKey => {
          ...
          if (dayData) {
            const stars = dayData.totalStars || 0;
            const contributors = dayData.totalContributors || 0;
            point[repoKey] = contributors > 0 ? stars / contributors : null;
          }
        });
        return point;
...
// Issues Opened / Closed Ratio static chart component
function IssuesRatioChart({ selectedRepos, repoData }) {
  const [datePreset, setDatePreset] = useState('all');
  const [customStartDate, setCustomStartDate] = useState('');
  const [customEndDate, setCustomEndDate] = useState('');
  const [viewMode, setViewMode] = useState('date');
    ...
    if (viewMode === 'indexed') {
      ...
      for (let i = 0; i < maxLength; i++) {
        ...
        selectedRepos.forEach(repoKey => {
          const data = filteredData[repoKey] || [];
          if (i < data.length) {
            const opened = data[i].totalIssuesOpened || 0;
            const closed = data[i].totalIssuesClosed || 0;
      ...
      return sortedDates.map(date => {
        ...
        selectedRepos.forEach(repoKey => {
          ...
          const dayData = data.find(d => d.date === date);
          if (dayData) {
            const opened = dayData.totalIssuesOpened || 0;
            const closed = dayData.totalIssuesClosed || 0;
            point[repoKey] = closed > 0 ? opened / closed : null;
          }
        });
        return point;
    ...
    if (selectedRepos.includes(repoKey)) {
      ...
      if (needsDaily || needsMonthly) {
        ...
        try {
          ...
          console.error(`Failed to load comparison data for ${repoKey}:`, error);
        } finally {
    ...
    <div>
      ...
      {selectedRepos.length > 0 && comparisonData.length > 0 && (
        <div className="mt-8">
          ...
          {/* Stars to Contributors Ratio Chart */}
          <StarsContributorsRatioChart
            selectedRepos={selectedRepos}
            repoData={repoData}
          />
