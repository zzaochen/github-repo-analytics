    ...
    <div>
      ...
      {recentlyAdded.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg p-6 mb-8 shadow-sm">
          ...
          <p className="text-gray-500 text-sm mb-4">New repos added from trending in the last 48 hours</p>
          <div className="flex flex-wrap gap-2">
            {recentlyAdded.map((repo) => (
              <a
                key={repo.id}
                href={`https://github.com/${repo.owner}/${repo.repo}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 px-3 py-1.5 bg-green-50 hover:bg-green-100 border border-green-200 rounded-full text-sm text-green-700 transition-colors"
                title={`Added ${new Date(repo.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}`}
              >
